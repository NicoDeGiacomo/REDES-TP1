...
class TCPSAckKReceiver(TCPSAck):
    ...
    def listen_packet(self):
        ...
        data, addr = self.socket.receive_message(1500, self.addr)#TODO catch timeout
    ...
    def send_ack_and_sack(self):
        ...
        left_seq_num = self.window[0].seq_num

        for i in range(1, len(self.window)):
            if not self.window[i - 1].header.seq_num == self.window[i].header.seq_num - 1:
                sack.append((left_seq_num, self.window[i - 1].header.seq_num + 1))
                left_seq_num = self.window[i].header.seq_num

        logger.debug(f"SACK formed with blocks: {sack}")
